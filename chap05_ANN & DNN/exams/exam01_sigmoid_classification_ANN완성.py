...
# Hidden layer
w1 = tf.Variable(tf.random_normal([2, hideen_nodes]))
b1 = tf.Variable(tf.random_normal([hideen_nodes]))

# Output layer
w2 = tf.Variable(tf.random_normal([hideen_nodes, 1]))
b2 = tf.Variable(tf.random_normal([1]))

hidden_output = tf.nn.relu(tf.matmul(X, w1) + b1)# hidden 계산
model = tf.matmul(hidden_output, w2) + b2 # output 계산 
